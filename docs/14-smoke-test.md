...
```
openstack security group rule create --dst-port=${NODE_PORT} kubernetes
```

Retrieve the external IP address of a worker instance:

```
EXTERNAL_IP=$(openstack server show kube-controller-1 -f value -c addresses|cut -d',' -f2|tr -d ' ')
```
